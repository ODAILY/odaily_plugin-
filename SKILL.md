...
version: 4.0.0
author: Odaily
metadata: {"openclaw":{"emoji":"📰","requires":{"bins":["python3","pip3"]},"install":[{"id":"pip","kind":"shell","package":"","label":"pip install -r requirements.txt"}]}}
---
...
每次输出任意单个或部分板块后，响应的**最后一行必须**是以下格式的追问提示：
...
**规则：**
- 只列出本次**未输出**的板块（已输出的不再列出）
- 必须包含每个板块的一句话说明，方便用户选择
- 全板块日报（5个板块都输出）时**不追加**此提示
- **无论用户问的是什么，只要是单板块输出，结尾必须有此追问，不得遗漏**
